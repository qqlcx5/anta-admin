...
```ts
import { cloneDeep, isAllEmpty, debounce, throttle, useDark, useWatermark } from "@/utils";
```
...
### 6. `color.ts` 颜色计算
```ts
import { darken, lighten, hexToRgb, rgbToHex, randomColor, randomGradient } from "@/utils";

darken("#409eff", 20);      // 颜色加深 20%
lighten("#409eff", 20);     // 颜色变浅 20%
